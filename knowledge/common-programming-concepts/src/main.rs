...
fn main() {
    ...
    // 4. 常量只能是常量表达式，而不能是函数调用返回值，或任何其他只在运行时计算的值，也就是说，常量的值编译时就确定的
    const MAX_POINTS: u32 = 100_000; // Rust 中可以使用 `_` 分隔符划分一个长数字而不影响数字的值，这样便于读数
    let abb = 7 + MAX_POINTS;
    ...
    // 关于字符型，占4个字节的大小，字符类型代表了一个 Unicode 标量值，意味着可以表示从 U+0000 to U+D7FF 以及 U+E000 to U+10FFFF 共计 1112064 个字符
    ...
    // 反向 for 循环
    // `a..b` 是一种范围表示法，会产生一列从 a（包含）到 b（不包含）的数，步进为1
    // `a..=b` 则包含两边
